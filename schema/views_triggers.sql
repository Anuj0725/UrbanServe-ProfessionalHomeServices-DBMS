-- ============================================================
-- 03_views_triggers.sql
-- Trigger, views, materialized views, and stored procedure.
-- Run AFTER 01_ddl.sql, seed.sql, and 02_indexes.sql.
-- ============================================================

-- ------------------------------------------------------------
-- TRIGGER: trg_update_provider_avg_rating
-- Keeps ServiceProvider.avg_rating in sync with ProviderReview.
-- Fires on INSERT, UPDATE, and DELETE so the stored value never
-- drifts from the underlying review data, no matter which
...
-- Full booking detail for a single read — customer, provider,
-- location, coupon, and payment status — across 7 tables.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- MATERIALIZED VIEW: mv_provider_leaderboard
-- Provider ranking by rating and review volume. Materialized
-- (not a plain view) because this aggregates across every review
-- and booking for every provider — expensive to recompute on
-- every page load, so it's precomputed and refreshed on demand.
...
-- ------------------------------------------------------------
-- MATERIALIZED VIEW: mv_city_revenue_summary
-- Revenue and booking stats per city. Same rationale as above —
-- this rolls up every booking and payment in the system, grouped
-- by city, so it's precomputed rather than calculated live.
...
-- ------------------------------------------------------------
-- STORED PROCEDURE: place_booking
-- Atomically inserts a Booking, its first BookingItem, and the
-- initial BookingStatusLog entry as one transactional unit.
-- If any insert fails, all three roll back together — no
-- half-created booking is ever left behind.
--
-- NOTE: booking_id/log_id are plain INT PKs in this schema (not
-- SERIAL), so the next ID is computed manually via MAX(id)+1
-- inside the function before inserting.
...
DECLARE
    ...
    v_log_id     INT;
BEGIN
    v_booking_id := (SELECT COALESCE(MAX(booking_id), 0) + 1 FROM Booking);
    ...
    INSERT INTO Booking (
        ...
        v_booking_id, p_customer_id, p_provider_id, p_address_id, p_coupon_id,
        p_scheduled_date, p_scheduled_time, p_total_amount,
        'Pending', p_special_instructions, CURRENT_TIMESTAMP
    );

    INSERT INTO BookingItem (booking_id, item_no, service_id, quantity, unit_price)
    VALUES (v_booking_id, 1, p_service_id, p_quantity, p_unit_price);

    v_log_id := (SELECT COALESCE(MAX(log_id), 0) + 1 FROM BookingStatusLog);

    INSERT INTO BookingStatusLog (log_id, booking_id, status, remarks, changed_at)
    VALUES (v_log_id, v_booking_id, 'Pending', 'Booking created — awaiting provider confirmation', CURRENT_TIMESTAMP);
...
-- Example call:
-- SELECT place_booking(1, 2, 3, NULL, '2026-07-01', '10:00', 599, 'Test booking', 5, 1, 699);
